/*
 * Rainbow Crackalack: test_chain_md5_9.c
...
struct md5_9_chain_test {
  uint64_t start;
  uint64_t end;
};

struct md5_9_chain_test md5_9_chain_tests[] = {
  {0UL,    282230011173483660UL},
  {456UL,  276411458831213344UL},
  {9999UL, 153969053690369043UL},
};


/* Test a chain using the CPU. */
int cpu_test_chain_md5_9(uint64_t start, uint64_t expected_end) {
  ...
  computed_end = generate_rainbow_chain(HASH_MD5, CHARSET_ASCII_32_95, CHARSET_ASCII_32_95_LEN, 9, 9, 0, 803000, start, plaintext_space_up_to_index, plaintext_space_total, plaintext, &plaintext_len, hash, &hash_len);
...
int test_chain_md5_9(gpu_device device, gpu_context context, gpu_kernel kernel) {
  int tests_passed = 1;
  unsigned int i = 0;


  for (i = 0; i < (sizeof(md5_9_chain_tests) / sizeof(struct md5_9_chain_test)); i++) {
    tests_passed &= gpu_test_chain_md5_9(device, context, kernel, md5_9_chain_tests[i].start, md5_9_chain_tests[i].end);
    tests_passed &= cpu_test_chain_md5_9(md5_9_chain_tests[i].start, md5_9_chain_tests[i].end);
  }
