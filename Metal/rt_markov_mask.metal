 ...
 * for Metal.
 *
 * Like index_to_plaintext_markov (rt_markov.metal) but each position i has its
 * own radix sizes[i] (the number of allowed characters at that mask position)
 * and consults restricted, per-position sorted tables:
 *
...
inline void index_to_plaintext_markov_mask(
    ...
    unsigned int max_sz,
    constant unsigned int *sizes,
    constant unsigned char *r_pos0,
    constant unsigned char *r_bigram,
    thread unsigned char *plaintext)
{
    ...
    for (unsigned int i = 1; i < mask_len; i++) {
        ulong off = ((ulong)i * charset_len + prev) * max_sz;
