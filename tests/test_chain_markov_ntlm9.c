/*
 * Rainbow Crackalack: test_chain_markov_ntlm9.c
 * GPU chain tests for the Markov NTLM9 fast-path kernel (crackalack_markov_ntlm9).
 *
 * The NTLM9 kernel hardcodes plaintext_len=9 and pspace=95^9, so we must
...
#define MC9_CHARSET_LEN    95
#define MC9_PLAINTEXT_LEN  9
#define MC9_MAX_POSITIONS  8
#define MC9_PSPACE_TOTAL   630249409724609375UL  /* 95^9 */

static const char MC9_CHARSET[] =
    " !\"#$%&'()*+,-./0123456789:;<=>?@"
    "ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`"
    "abcdefghijklmnopqrstuvwxyz{|}~";

struct markov_chain_test_ntlm9 {
    uint64_t     start;
    unsigned int chain_len;
    unsigned int table_index;
};

static struct markov_chain_test_ntlm9 ntlm9_tests[] = {
    {0UL,      100, 0},
    {1UL,      100, 0},
    {1000UL,    50, 1},
    {12345UL,  100, 0},
...
/*
 * CPU reference Markov chain generator for 9-char NTLM.
 */
static uint64_t cpu_markov_chain_ntlm9(uint64_t start, unsigned int chain_len,
                                        unsigned int table_index,
                                        const markov_model *model)
{
    ...
    for (unsigned int pos = 0; pos < chain_len - 1; pos++) {
        ...
        index_to_plaintext_markov_cpu(index, model, MC9_PLAINTEXT_LEN, plaintext);
        ntlm_hash((char *)plaintext, MC9_PLAINTEXT_LEN, hash);
        index = hash_to_index(hash, 16, reduction_offset, MC9_PSPACE_TOTAL, pos);
    }
    return index;
}
...
/*
 * GPU test: call crackalack_markov_ntlm9 kernel and compare endpoint.
 *
 * Arg layout matches the generic crackalack_markov kernel (14 args),
 * but the NTLM9 fast path ignores most of them.
 */
static int gpu_test_markov_chain_ntlm9(gpu_device device, gpu_context context,
                                        gpu_kernel kernel,
                                        const struct markov_chain_test_ntlm9 *t,
                                        ...
                                        const markov_model *model)
{
    ...
    gpu_uint charset_len_val  = MC9_CHARSET_LEN;
    gpu_uint plen_min         = MC9_PLAINTEXT_LEN;
    gpu_uint plen_max         = MC9_PLAINTEXT_LEN;
    ...
    gpu_ulong pspace_total    = MC9_PSPACE_TOTAL;
    ...
    if (!indices) { fprintf(stderr, "gpu_test_markov_chain_ntlm9: OOM\n"); exit(-1); }
    ...
    CLCREATEARG_ARRAY(1, charset_buf,   CL_RO, MC9_CHARSET,      MC9_CHARSET_LEN);
    ...
    CLCREATEARG_ARRAY(11, sorted_pos0_buf, CL_RO, model->sorted_pos0,
                      MC9_CHARSET_LEN * sizeof(uint8_t));
    CLCREATEARG_ARRAY(12, sorted_bigram_buf, CL_RO, model->sorted_bigram,
                      MC9_MAX_POSITIONS * MC9_CHARSET_LEN * MC9_CHARSET_LEN * sizeof(uint8_t));
...
int test_chain_markov_ntlm9(gpu_device device, gpu_context context, gpu_kernel kernel)
{
    int tests_passed = 1;
    unsigned int i;
    unsigned int num_tests = (unsigned int)(sizeof(ntlm9_tests) /
                                            sizeof(ntlm9_tests[0]));

    /* Build a synthetic Markov model with charset_len=95, max_positions=8. */
    size_t bigram_count = (size_t)MC9_MAX_POSITIONS * MC9_CHARSET_LEN * MC9_CHARSET_LEN;

    uint64_t *pos0_freq   = malloc(MC9_CHARSET_LEN * sizeof(uint64_t));
    uint64_t *bigram_freq = malloc(bigram_count * sizeof(uint64_t));
    if (!pos0_freq || !bigram_freq) {
        fprintf(stderr, "test_chain_markov_ntlm9: OOM\n");
        free(pos0_freq);
        free(bigram_freq);
        return 0;
    }
    ...
    for (unsigned int c = 0; c < MC9_CHARSET_LEN; c++)
    ...
    markov_model m;
    memset(&m, 0, sizeof(m));
    m.charset_len   = MC9_CHARSET_LEN;
    m.max_positions = MC9_MAX_POSITIONS;
    memcpy(m.charset, MC9_CHARSET, MC9_CHARSET_LEN);
    m.pos0_freq     = pos0_freq;
    m.bigram_freq   = bigram_freq;
    m.sorted_pos0   = malloc(MC9_CHARSET_LEN * sizeof(uint8_t));
    ...
    for (i = 0; i < num_tests; i++) {
        const struct markov_chain_test_ntlm9 *t = &ntlm9_tests[i];
        uint64_t cpu_end = cpu_markov_chain_ntlm9(t->start, t->chain_len,
                                                    t->table_index, &m);
        tests_passed &= gpu_test_markov_chain_ntlm9(device, context, kernel,
                                                      t, cpu_end, &m);
    }
