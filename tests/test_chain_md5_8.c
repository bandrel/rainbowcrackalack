/*
 * Rainbow Crackalack: test_chain_md5_8.c
...
struct md5_8_chain_test {
  uint64_t start;
  uint64_t end;
};

struct md5_8_chain_test md5_8_chain_tests[] = {
  {0UL,    4679021263755342UL},
  {456UL,  95915044935770UL},
  {9999UL, 2949033278493487UL},
};


/* Test a chain using the CPU. */
int cpu_test_chain_md5_8(uint64_t start, uint64_t expected_end) {
  ...
  plaintext_space_total = fill_plaintext_space_table(CHARSET_ASCII_32_95_LEN, 8, 8, plaintext_space_up_to_index);

  computed_end = generate_rainbow_chain(HASH_MD5, CHARSET_ASCII_32_95, CHARSET_ASCII_32_95_LEN, 8, 8, 0, 422000, start, plaintext_space_up_to_index, plaintext_space_total, plaintext, &plaintext_len, hash, &hash_len);
...
int gpu_test_chain_md5_8(gpu_device device, gpu_context context, gpu_kernel kernel, uint64_t start, uint64_t expected_end) {
  CLMAKETESTVARS();
  int test_passed = 0;
  ...
  gpu_uint unused_uint_arr[MAX_PLAINTEXT_LEN] = {0};
  char unused_char[16] = {0};
  gpu_ulong *start_array = NULL, *end_array = NULL;
  gpu_uint chain_len = 422000, pos_start = 0;
  ...
  if (end_array[0] == expected_end)
    ...
    printf("\n\n\tStart:        %"PRIu64"\n\tExpected end: %"PRIu64"\n\tComputed end: %"PRIu64"\n\n", start, expected_end, end_array[0]);
  ...
  CLFREEBUFFER(unused9_buf);
  CLFREEBUFFER(unused10_buf);
...
int test_chain_md5_8(gpu_device device, gpu_context context, gpu_kernel kernel) {
  ...
  for (i = 0; i < (sizeof(md5_8_chain_tests) / sizeof(struct md5_8_chain_test)); i++) {
    tests_passed &= gpu_test_chain_md5_8(device, context, kernel, md5_8_chain_tests[i].start, md5_8_chain_tests[i].end);
    tests_passed &= cpu_test_chain_md5_8(md5_8_chain_tests[i].start, md5_8_chain_tests[i].end);
  }
