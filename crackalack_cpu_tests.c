 ...
 *
 * Standalone CPU-only test binary.  Runs the 7 CPU-only unit tests without
 * any GPU initialization.  Designed for CI environments that have no GPU
 * (only build-essential, libgcrypt, and OpenCL headers are required).
 *
