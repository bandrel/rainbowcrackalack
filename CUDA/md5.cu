...
/*
 * MD5 implementation for CUDA kernels.
 * Based on RFC 1321: https://www.ietf.org/rfc/rfc1321.txt
 * Uses MD5_ prefixed macros to avoid conflicts with ntlm.cu MD4 macros.
 */
...
__device__ inline void md5_compress(uint *state, uint *W) {
...
__device__ inline void md5_hash(unsigned char *plaintext, unsigned int plaintext_len,
                                unsigned char *hash) {
  uint W[16] = {0};
