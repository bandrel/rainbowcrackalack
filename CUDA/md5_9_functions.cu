...
__device__ inline void index_to_plaintext_md5_9(unsigned long long index,
                                                 const char *charset,
...
__device__ inline unsigned long long hash_md5_9(unsigned char *plaintext) {
  uint W[16] = {0};
  ...
  return (unsigned long long)state[0] | ((unsigned long long)state[1] << 32);
}


__device__ inline unsigned long long hash_to_index_md5_9(unsigned long long hash, unsigned int reduction_offset, unsigned int pos) {
  return (hash + reduction_offset + pos) % 630249409724609375ULL;  /* 95^9 */
}


__device__ inline unsigned long long hash_char_to_index_md5_9(unsigned char *hash_value,
                                                               unsigned int reduction_offset,
                                                               unsigned int pos) {
  unsigned long long ret = hash_value[7]; ret <<= 8;
  ...
  ret |= hash_value[0];
  return (ret + reduction_offset + pos) % 630249409724609375ULL;
}
