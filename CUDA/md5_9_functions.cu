...
__device__ inline unsigned long long hash_to_index_md5_9(unsigned long long hash, unsigned int pos) {
  return (hash + pos) % 630249409724609375ULL;  /* 95^9 */
}
...
__device__ inline unsigned long long hash_char_to_index_md5_9(unsigned char *hash_value,
  ...
  ret |= hash_value[0];
  return (ret + pos) % 630249409724609375ULL;
}
