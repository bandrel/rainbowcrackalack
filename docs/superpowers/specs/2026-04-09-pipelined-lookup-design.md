...
Two-phase pipeline with an outer loop for table chunks that fit in RAM (~96GB on gpuhost3, ~4GB reserved for overhead):
...
2. **Memory:** Verify RAM budget calculation works on gpuhost3 (96GB). Load ~75 tables, confirm no OOM.
