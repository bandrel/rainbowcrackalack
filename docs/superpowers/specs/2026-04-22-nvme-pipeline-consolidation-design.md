...
2. Replace hardcoded Synology paths with a config-file-driven approach that's portable across dell2/dell3 (and reproducible for any rainbowcrackalack user handling Mandiant tables).
...
- No multi-host orchestration across dell2 and dell3 in this pass (state dir is host-local; cross-host is a future problem).
...
- **Bash associative arrays for done-file membership** — O(1) startup load vs. the current O(n²) grep loop. Requires bash 4+, which is fine on Linux. Would break macOS's default bash 3.2, but the pipeline never runs on a Mac (lookup targets dell2/dell3 Linux hosts).
...
- Multi-host state-dir sharing across dell2 + dell3 (would need a shared NFS path for `$RC_STATE_DIR` and real flock semantics across NFS).
