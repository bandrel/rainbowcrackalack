...
Recent NetNTLMv1 work merged to bandrel `master` touches chain math and the
...
`fix/precompute-cache-challenge-key`. A hash that should have cracked on dell3
...
run_regression.sh crackdiff   # Oracle 2: BASE-vs-CANDIDATE differential cracking (dell3, real tables)
...
Backend-agnostic; runs on **dell3 (CUDA)** and **local Metal (M3 Max)**. For
...
The identical known hashes are run on CUDA (dell3) and Metal (M3 Max). A path
...
dell3 only, against the **real** `/mnt/nvme/rtc/` NetNTLMv1-7 tables.
