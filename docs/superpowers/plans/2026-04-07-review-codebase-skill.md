...
**Files:**
- Create: `~/.agent/skills/review-codebase/skill.md`
...
Run:
```bash
mkdir -p ~/.agent/skills/review-codebase
```

- [ ] **Step 2: Write the skill file with frontmatter and label setup**

Create `~/.agent/skills/review-codebase/skill.md` with the following content:
...
- GitLab instance at `https://git.example.com` must be reachable
...
```bash
curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  "https://git.example.com/api/v4/projects/5/issues?labels=auto-review&state=opened&per_page=100" \
  | python3 -c "
import sys, json
...
- If `verdict` is `"false_positive"`: close the issue with a comment:
  ...
  curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
    ...
    "https://git.example.com/api/v4/projects/5/issues/ISSUE_IID/notes"

  # Close the issue
  curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
    --header "Content-Type: application/json" \
    --data '{"state_event": "close"}' \
    "https://git.example.com/api/v4/projects/5/issues/ISSUE_IID"
  ```
...
Run:
```bash
ls -la ~/.agent/skills/review-codebase/skill.md
```
...
Note: The skill file lives in `~/.agent/skills/` (outside the repo), so it isn't committed to the project.
...
```bash
curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  "https://git.example.com/api/v4/projects/5/labels?per_page=100" \
...
curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  ...
  "https://git.example.com/api/v4/projects/5/issues" \
...
curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  ...
  "https://git.example.com/api/v4/projects/5/issues/IID/notes" \
...
curl -sf --header "PRIVATE-TOKEN: $GITLAB_TOKEN" \
  --header "Content-Type: application/json" \
  --data '{"state_event": "close"}' \
  "https://git.example.com/api/v4/projects/5/issues/IID" \
...
Then create `~/.agent/skills/review-codebase/skill.md` with the full content from Task 1, Step 2 (the complete skill.md content within the code fence).

- [ ] **Step 2: Verify skill file**

Run:
```bash
head -5 ~/.agent/skills/review-codebase/skill.md
```
...
```bash
cd /path/to/projects/rainbowcrackalack
