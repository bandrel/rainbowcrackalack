...
- `cuda_setup.c` only compiles on a CUDA host. **Tasks touching `cuda_setup.c` must be compile-verified on a CUDA box (gpuhost3), not on this Mac.** Run there: `make clean && make`.
...
- Runtime validation (the actual co-run-with-hashcat test) happens on gpuhost3 — see Task 6.
...
- [ ] **Step 2: Compile-verify on a CUDA box (gpuhost3)**
...
On gpuhost3 (exercises CUDA impl): `make clean && make` — expected: builds clean.
...
- [ ] **Step 2: Compile-verify on gpuhost3**
...
On gpuhost3: `make clean && make` — expected: builds clean.
...
## Task 6: End-to-end validation on gpuhost3 (manual)

GPU OOM behavior cannot be unit-tested; this is the acceptance test. No code changes.

- [ ] **Step 1: Build on gpuhost3**

Run: `make clean && make` (with the correct `CUDA_PATH` per the gpuhost2/gpuhost3 build notes).
