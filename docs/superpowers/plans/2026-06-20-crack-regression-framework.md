...
**Goal:** Build an automated framework that proves cracking has no false negatives (self-contained round-trip) and no regressions (BASE-vs-CANDIDATE differential) for the NTLM-8, NTLM-9, and NetNTLMv1-7 (default-challenge) paths, runnable on gpuhost3 (CUDA) and local Metal.
...
execution on a GPU host (gpuhost3 and/or local Metal). The `roundtrip` phase uses
...
#   run_regression.sh crackdiff   # BASE-vs-CANDIDATE differential (gpuhost3 real tables)
...
- [ ] **Step 2: Run crackdiff on gpuhost3**

Run on gpuhost3 (with `REAL_TABLES` pointing at a small subset directory for a first pass):
```bash
